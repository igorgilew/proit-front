...
class WorkerAddComp extends React.Component{
    ...
    getWorkersByIdOrg(){
        ...
        WorkerApi.getWorkersByOrgIdd(this.state.orgIdd)
            .then(response => {
                if (response.status === 200) {
                    this.setState({bosses: response.data["list"]});
                    this.setState({errorMessage: null});
                } else {
                    this.setState({errorMessage: response.data.message});
                }
    ...
    onOrgSelectChange = (e)=>{
        this.setState({orgIdd: e.target.value, bossIdd: ""}, ()=>{
            if(this.state.orgIdd !== "")
                this.getWorkersByIdOrg(this.state.orgIdd);
            else{
    ...
    onSaveClick = ()=>{
        ...
        if(this.state.orgIdd !== ""){
            WorkerApi.create(data)
                .then(response=>{
                    if (response.status===200){
                        this.setState({workerFirstName: ""});
                        this.setState({workerSecondName: ""});
                        this.setState({orgIdd: ""});
                        this.setState({bossIdd: ""});
                        this.setState({errorMessage: null});
